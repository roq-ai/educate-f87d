...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiGlobe } from 'react-icons/fi';
...
import { PublicInterface } from 'interfaces/public';
import { getPublics } from 'apiSdk/publics';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['students', 'healthcare-professional', 'medical-educator', 'emergency-responder'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PublicInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/publics` : null),
    () => getPublics({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
